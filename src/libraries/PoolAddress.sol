...
library PoolAddress {
    ...
    */
    function computeAddress(address _factory, address _token0, address _token1, uint24 _fee)
        internal
        pure
        returns (address pre_deployed_pool)
    {
        ...
        //                     _factory,
        //                     keccak256(abi.encodePacked(_token0, _token1, _fee)),
        //                     keccak256(type(UniswapV3SimulatorPool).creationCode) // hashed contract code is needed to protect from hash-collisions(salt).
        //                 )
        //             )
        //         )
        //     )
        ...
        pre_deployed_pool = Create2.computeAddress(
            keccak256(type(UniswapV3SimulatorPool).creationCode),
            keccak256(abi.encodePacked(_token0, _token1, _fee)),
            _factory
        );
    }
