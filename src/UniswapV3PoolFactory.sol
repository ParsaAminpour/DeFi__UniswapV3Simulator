...
contract UniswapV3SimulatorPoolFactory is IUniswapV3PoolDeployer {
    ...
    mapping(address token0 => mapping(
        address token1 => mapping(
            uint24 tickSpace => address pool))) private pools;
    ...
    function createPool(address _token0, address _token1, uint24 _tickSpacing) external returns(address pool) {
        ...
        if (_token0 == address(0) || _token0 == address(0)) revert UniswapV3SimulatorPoolFactory__ZeroAddressNotAllowed();
        ...
        pool = address(
            new UniswapV3SimulatorPool{salt:keccak256(abi.encodePacked(_token0, _token1, _tickSpacing))}()
        );
