...
abstract contract PoolHandler is Test {
    ...
    function deployPool(
        UniswapV3SimulatorPoolFactory factory,
        address token0,
        address token1,
        uint24 fee,
        uint256 currentPrice
    ) internal returns (UniswapV3SimulatorPool pool) {
        pool = UniswapV3SimulatorPool(factory.createPool(token0, token1, fee));
        pool.pool_initialize(sqrtP(currentPrice));
    }
